...
namespace YouseiAR.Missions
{
    ...
    public class MissionUIBinder : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            // 既にミッションが割り当てられている場合はUIを即時更新する
            if (missionManager.Current != null)
            {
                HandleMissionChanged(missionManager.Current);
                if (missionManager.State != null)
                {
                    HandleProgress(missionManager.State.progress);
                    if (missionManager.State.completed)
                    {
                        HandleCompleted(missionManager.Current);
                    }
                }
            }
        }
